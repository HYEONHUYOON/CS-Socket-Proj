    ...
    class TCPServer
    {
        ...
        public void Accept(List<string> ipList)
        {
            try
            {
                ...
                if (!isWhiteList)
                {
                    bytes = Encoding.Default.GetBytes("403");
                ...
                {
                    bytes = Encoding.Default.GetBytes("WELCOME");
                    client_sock.Send(bytes, bytes.Length, SocketFlags.None);
        ...
        public void ClientSockClose()
        {
            try
            {
                ...
                buf = Encoding.Default.GetBytes("GOODBYE ");
                client_sock.Send(buf, buf.Length, SocketFlags.None);

                sw.WriteLine(DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss") + " => "
                + $"{"SEND",-15} IP : {clientaddr.Address,-15} PORT : {clientaddr.Port,-10} {buf}");
                sw.Flush();
        ...
        public void httpProtocolParse(string protocol)
        {
